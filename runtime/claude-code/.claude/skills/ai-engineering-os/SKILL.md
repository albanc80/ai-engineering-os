...
1. Use specialized project subagents from `.claude/agents/` for repository analysis, architecture, implementation, testing, security, adversarial review, and release review when available.
2. Keep exploration-heavy work in read-only subagents to protect the main context.
3. For independent review, delegate to a reviewer agent that did not implement the change.
4. Treat hooks as deterministic controls, not as reasoning substitutes.
5. Before destructive commands or release actions, follow repository permission policy and human gates.
6. Use `.ai-engineering/state.json` as the machine-readable workflow state when the project opts into durable state.
7. If project `CLAUDE.md` conflicts with this skill, apply the stricter safety/review requirement and surface the conflict.
